// src/app/page.tsx
import { Search, User, ArrowRight, Icon } from 'lucide-react';
  ...
  return (
    <div className="h-screen bg-white flex flex-col items-center">
      ...
      <main className="container mx-auto p-4 md:p-8 mt-20 flex flex-col items-center">
        ...
        <h1 className="text-4x1 sm:text-6xl font-Inter text-black/50 text-center mb-">
          อยากกินอะไร เราหาให้
        </h1>

        {/* Gallery Layout with 70% scale and proper centering */}
        <div className="w-full bg-white font-sans flex justify-center ">
          <div className="relative w-[1330px] h-auto">
            
            {/* รูปที่ 2: อยู่ข้างซ้ายของรูปที่ 3 */}
            <div className="absolute w-[300px] h-[242.9px] top-[100px] left-[-90px] rounded-[40px] overflow-hidden border-t-5 border-l-5 border-solid border-green-400">
              <Image src={imageUrls.img2} alt="Food 2" layout="fill" objectFit="cover" />
            </div>

            {/* รูปที่ 1: อยู่ข้างล่างรูปที่ 2 */}
            <div className="absolute w-[300px] h-[116.2px] top-[360px] left-[-90px] rounded-[40px] overflow-hidden border-t-5 border-l-5 border-solid border-green-400">
              <Image src={imageUrls.img1} alt="Food 1" layout="fill" objectFit="cover" />
            </div>

            {/* รูปที่ 3: อยู่ซ้ายมือของรูปที่ 4 */}
            <div className="absolute w-[227.5px] h-[386.4px] top-[50px] left-[220px] rounded-[40px] overflow-hidden border-t-5 border-l-5 border-solid border-green-400">
              <Image src={imageUrls.img3} alt="Food 3" layout="fill" objectFit="cover" />
            </div>

            {/* รูปที่ 4: อยู่ตรงกลาง */}
            <div className="absolute w-[300px] h-[270px] top-[50px] left-[460px] rounded-[40px] overflow-hidden border-t-5 border-l-5 border-solid border-green-400">
              <Image src={imageUrls.img4} alt="Food 4" layout="fill" objectFit="cover" />
            </div>

            {/* รูปที่ 5: อยู่ด้านขวาของรูปที่ 4 */}
            <div className="absolute w-[227.5px] h-[386.4px] top-[50px] left-[767.9px] rounded-[40px] overflow-hidden border-t-5 border-l-5 border-solid border-green-400">
              <Image src={imageUrls.img5} alt="Food 5" layout="fill" objectFit="cover" />
            </div>
            
            {/* รูปที่ 7: อยู่ขวามือของรูปที่ 5 */}
            <div className="absolute w-[298.9px] h-[242.9px] top-[100px] left-[1008.7px] rounded-[40px] overflow-hidden border-t-5 border-l-5 border-solid border-green-400">
              <Image src={imageUrls.img7} alt="Food 7" layout="fill" objectFit="cover" />
            </div>

            {/* รูปที่ 6: อยู่ข้างล่างของรูปที่ 7 */}
            <div className="absolute w-[298.9px] h-[116.2px] top-[360px] left-[1008.7px] rounded-[40px] overflow-hidden border-t-5 border-l-5 border-solid border-green-400">
